  ...
  async function submit(e: MouseEvent<HTMLButtonElement>) {
    ...
    try {
      ...
      if (data.status === 201) {
        ...
        alertService.error(
          "Adding network failed with: ".concat(await data.text()),
          {},
        );
      }
    } catch (error) {
          ...
          <Modal.Body>
            <Form.Group>
              <Form.Group className="mb-3" controlId="formName">
                ...
                <Form.Control
                  type="name"
                  placeholder="Network name, eg: Dummy Network"
                  name="name"
                  onChange={onChangeSetObject}
              ...
              <Form.Group className="mb-3" controlId="formCIDR">
                ...
                <Form.Control
                  type="networkaddress"
                  placeholder="Network CIDR, eg: 10.1.0.0/24"
                  name="cidr"
                  onChange={onChangeSetObject}
              ...
              <Form.Group
                className="mb-3"
                controlId="formConnectionConfiguration"
              >
                <Form.Label>Network CIDR</Form.Label>
                <Form.Control
                  as="textarea"
                  rows={3}
                  placeholder="This must be the connection configuration token"
                  name="connection_config"
                  onChange={onChangeSetObject}
                  defaultValue={network.connection_config}
                  required
                />
              </Form.Group>
            </Form.Group>
